...
def importPriceDolar():
    def loadCache(path_dir):
        if os.path.exists(path_dir):
            try:
                ...
                option= input("There is no data in the database. However, the data will be saved, do you want to continue? y/n: ")
    ...
    datas_cache = loadCache(path_dir)
    ...
    if datas_cache:
        ...
        if price_dolar and date_submitted:
            ...
            if date_submitted_dt <= date_cache:
                ...
                saveCache(path_dir, price_dolar, date_submitted)
                return price_dolar
        else:
